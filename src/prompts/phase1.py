...
2. **기독론적 연결**: 본문의 메시지가 사중복음(중생, 성결, 신유, 재림) 중 어떤 요소와 가장 깊이 있고 자연스럽게 연결되는가? (예수 그리스도와의 결점)
...
def get_phase1_prompt(
    ...
    Args:
        ...
        sermon_history: 최근 설교 이력 요약 (7순위 히스토리 참조)
...
"""
    history_block = f"\n{sermon_history}" if sermon_history else ""
    return f"""
